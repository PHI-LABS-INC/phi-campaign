...
const main = async () => {
  ...
  const aggregateByAddress: { [address: string]: number } = participants.reduce(
    (prev, addr) => ({ ...prev, [addr]: 0 }),
    {}
  );
  await Promise.all(
    Object.keys(aggregateByENS).map(async (ens) => {
      const addr = (
        await retry<string>(() => contract.ownerOfPhiland(ens.slice(0, -4)), 3)
      ).toLowerCase();
